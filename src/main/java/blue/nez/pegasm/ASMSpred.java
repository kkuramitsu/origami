...
public final class ASMSpred extends AbstractTableInstruction {
	...
	public ASMSpred(SymbolPredicate pred, Symbol label, Object option, PegAsmInstruction next) {
		super(label, next);
		this.pred = pred;
		this.option = option;
	}

	public ASMSpred(SymbolPredicate pred, Symbol label, PegAsmInstruction next) {
	...
	public PegAsmInstruction exec(PegAsmContext<?> sc) throws ParserTerminationException {
		return this.pred.match(sc, this.label, sc.xPPos(), this.option) ? this.next : sc.xFail();
	}
}