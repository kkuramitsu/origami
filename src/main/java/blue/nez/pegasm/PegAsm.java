...
public class PegAsm {
	...
	static Object opValue(PegAsmInstruction inst, int p) {
	...
	public static void stringfy(PegAsmInstruction inst, StringBuilder sb) {
		...
		for (int i = 0; i < size; i++) {
			...
			case "jump":
				...
				sb.append("L" + ((PegAsmInstruction) value).id);
				break;
			case "utf8":
	...
	public static PegAsmInstruction joinPoint(PegAsmInstruction inst) {
		if (inst != null) {
			inst.joinPoint = true;
		}
		return inst;
	...
	static abstract class AbstByte extends PegAsmInstruction {
		public final int byteChar;

		AbstByte(int byteChar, PegAsmInstruction next) {
			super(next);
	...
	static abstract class AbstAny extends PegAsmInstruction {
		AbstAny(PegAsmInstruction next) {
			super(next);
		}
	...
	static abstract class AbstStr extends PegAsmInstruction {
		public final byte[] utf8;

		public AbstStr(byte[] utf8, PegAsmInstruction next) {
			super(next);
			this.utf8 = utf8;
		}
	}
	...
	static abstract class AbstractTableInstruction extends PegAsmInstruction {
		public final Symbol label;

		AbstractTableInstruction(Symbol label, PegAsmInstruction next) {
			super(next);
			this.label = label;
		}
	...
	static abstract class AbstMemo extends PegAsmInstruction {
		final MemoPoint memoPoint;
		public final int uid;
		public final boolean state;
		public final PegAsmInstruction jump;

		AbstMemo(MemoPoint m, boolean state, PegAsmInstruction next, PegAsmInstruction skip) {
			super(next);
			this.memoPoint = m;
			this.uid = m.id;
		...
		AbstMemo(MemoPoint m, boolean state, PegAsmInstruction next) {
			super(next);
			this.memoPoint = m;
			this.uid = m.id;
			this.state = state;
