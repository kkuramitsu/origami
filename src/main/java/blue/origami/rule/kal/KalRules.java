...
public class KalRules implements OImportable, OSymbols, SyntaxAnalysis {
		...
		public OCode typeRule(OEnv env, Tree<?> t) {
			for (OEnv cur = env.getParent(); cur != null; cur = cur.getParent()) {
				if (cur.getSingletonClass() != null) {
					KalRules.this.setDefiningEnv(env, cur);
				}
			}
			return KalRules.this.typeExpr(env, t.get(_body));
		}
	};
	...
	public OTypeRule AssumeDecl = new TypeRule() {
		...
		public OCode typeRule(OEnv env, Tree<?> a) {
			OEnv defineEnv = KalRules.this.getDefiningEnv(env);
			for (Tree<?> t : a.get(_body)) {
				OType type = KalRules.this.parseType(env, t.get(_type));
				String[] names = KalRules.this.parseNames(env, t.get(_name));
	...
	public OTypeRule AssertExpr = new TypeRule() {
		...
		public OCode typeRule(OEnv env, Tree<?> t) {
			...
			OCode expr = KalRules.this.typeCondition(env, t.get(_cond));
	...
	public class InlineAssertRule extends TypeRule {
		...
		@Override
		public OCode typeRule(OEnv env, Tree<?> t) {
			OCode left = KalRules.this.typeExpr(env, t.get(_left));
			OCode right = KalRules.this.typeExpr(env, t.get(_right));
	...
	public OTypeRule MutableExpr = new TypeRule() {
		@Override
		public OCode typeRule(OEnv env, Tree<?> t) {
			OCode expr = KalRules.this.typeExpr(env, t.get(_expr));
	...
	public OTypeRule EmptyExpr = new TypeRule() {
		...
		public OCode typeRule(OEnv env, Tree<?> t) {
			OType ty = null;
			if (t.has(_type)) {
				ty = KalRules.this.parseType(env, t.get(_type));
			} else {
				ty = KalRules.this.typeExpr(env, t.get(_expr)).getType();
			}
	...
	public class ArrayRule extends TypeRule {
		...
		public OCode typeRule(OEnv env, Tree<?> t) {
			...
			for (int i = 0; i < t.size(); i++) {
				OCode element = KalRules.this.typeExpr(env, t.get(i));
				if (ctype.isUntyped()) {
			...
			if (!ctype.isUntyped()) {
				for (int i = 0; i < l.size(); i++) {
					l.set(i, KalRules.this.typeCheck(env, ctype, l.get(i)));
				}
			}
	...
	public class DictRule extends TypeRule {
		...
		public OCode typeRule(OEnv env, Tree<?> t) {
			...
			for (int i = 0; i < t.size(); i++) {
				...
				OCode element = KalRules.this.typeExpr(env, e.get(_value));
				if (vtype.isUntyped()) {
			...
			if (vtype.isUntyped()) {
				...
				for (int i = 1; i < keys.size(); i += 2) {
					keys.set(i, KalRules.this.typeCheck(env, vtype, keys.get(i)));
				}
			}
	...
	public OTypeRule ForEachExpr = new TypeRule() {
		...
		public OCode typeRule(OEnv env, Tree<?> t) {
			...
			OCode iterCode = OrigamiIterator.newIteratorCode(env, KalRules.this.ensureTypedExpr(env, t.get(_expr)));
			...
			OCode bodyCode = KalRules.this.typeExprOrErrorCode(lenv, t.get(_body));
	...
	public OTypeRule ClassDecl = new TypeRule() {
		...
		public OCode typeRule(OEnv env, Tree<?> t) {
			/* annotations */
			OAnno anno = KalRules.this.parseAnno(env, "public", t.get(_anno, null));
			String name = t.getText(_name, null);

			/* extends */
			OType superType = env.t(Object.class);
			if (t.has(_super)) {
				superType = KalRules.this.parseType(env, t.get(_super), superType);
			}

			/* implements */
			OType[] interfaces = KalRules.this.parseInterfaceTypes(env, t.get(_impl, null));

			OType[] params = null;
			OClassDeclType ct = new OClassDeclType(env, anno, name, null, superType, interfaces);
			ct.getDecl().addBody(t.get(_body, null));
			KalRules.this.defineName(env, t, ct);

			if (t.has(_param)) {
				OTypeSystem ts = env.getTypeSystem();
				String[] paramNames = KalRules.this.parseParamNames(env, t.get(_param, null));
				OType[] paramTypes = KalRules.this.parseParamTypes(env, paramNames, t.get(_param, null),
						env.t(AnyType.class));
	...
	public OTypeRule MethodDecl = new TypeRule() {
		...
		@Override
		public OCode typeRule(OEnv env, Tree<?> t) {
			OClassDecl cdecl = KalRules.this.checkClassContext(env);
			OAnno anno = KalRules.this.parseAnno(env, "public", t.get(_anno, null));
			OType rtype = KalRules.this.parseType(env, t.get(_type, null), env.t(OUntypedType.class));

			String name = t.getText(_name, "");
			String[] paramNames = KalRules.this.parseParamNames(env, t.get(_param, null));
			OType[] paramTypes = KalRules.this.parseParamTypes(env, paramNames, t.get(_param, null),
					env.t(AnyType.class));
			OType[] exceptions = KalRules.this.parseExceptionTypes(env, t.get(_throws, null));

			OCode body = KalRules.this.parseFuncBody(env, t.get(_body, null));
			OMethodHandle m = cdecl.addMethod(anno, rtype, name, paramNames, paramTypes, exceptions, body);
			if (anno.isStatic()) {
				KalRules.this.defineName(env, t, m);
			} else {
				KalRules.this.defineName(env, t, new OPartialFunc(m, 0, new JavaThisCode(cdecl.getType())));
			}
			return new EmptyCode(env);
		}
