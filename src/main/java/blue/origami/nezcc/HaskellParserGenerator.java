...
public class HaskellParserGenerator extends ParserSourceGenerator {
	...
	protected void initSymbols() {
		...
		this.defineSymbol("Tf", "ShortByteString -> Maybe (NezAST, String)");
		...
		this.defineSymbol("Tval", "Bool");
		this.defineSymbol("data", "val");
	...
	protected void writeHeader() throws IOException {
		this.open(this.getFileBaseName() + ".hs");
		this.writeResource("/blue/origami/nezcc/hsparser-libs.hs");
	...
	protected void declFuncType(String ret, String typeName, String... params) {
		StringBuilder sb = new StringBuilder();
		sb.append(String.format("type %s = ", typeName));
		int c = 0;
		for (String f : params) {
			String n = f.replace("?", "");
			if (c > 0) {
	...
	protected String emitArrayIndex(String a, String index) {
		return String.format("(unpack %s) !! %s", a, index);
	}
