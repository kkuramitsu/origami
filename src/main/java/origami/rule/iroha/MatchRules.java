...
public class MatchRules implements OImportable, OSymbols, SyntaxAnalysis {
	...
	public static abstract class ICase implements Case, OStringOut {
		String name;
		OType type;
		boolean nullAble;
	...
	static class TypeCase extends ICase {
		...
		@Override
		public void strOut(StringBuilder sb) {
			OStringOut.append(sb, this.getType());
			if (inner != null) {
				OStringOut.append(sb, inner);
			}
		}
	...
	static class ValueCase extends ICase {
		...
		@Override
		public void strOut(StringBuilder sb) {
			OStringOut.appendQuoted(sb, this.value);
		}
	...
	static class ValuesCase extends ICase {
		...
		public void strOut(StringBuilder sb) {
			for (int i = 0; i < values.length; i++) {
				...
				OStringOut.appendQuoted(sb, this.values[i]);
			}
		}
	...
	static class DRangeValue extends ICase {
		...
		public void strOut(StringBuilder sb) {
			sb.append("(");
			OStringOut.appendQuoted(sb, this.start);
			sb.append(" to ");
			if (!inclusive) {
				sb.append("<");
			}
			OStringOut.appendQuoted(sb, this.end);
			sb.append(")");
		}
